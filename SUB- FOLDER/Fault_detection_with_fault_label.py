...
data = pd.read_csv("Training_Data_F-.csv")
...
def predict_new_data(file_path):
    ...
    for model_file in model_files:
        ...
        prob_normal = proba[:, 1]*100  # Probability of Normal
        prob_warning = proba[:, 2]*200  # Probability of Warning
        prob_fault = proba[:, 0]*300  # Probability of Fault
    ...
    output_file = os.path.join(output_dir, "test_data_F-.csv")
    results.to_csv(output_file, index=False)
    print(f"\nPredictions for all models saved to: {output_file}")
    

predict_new_data("TEST/Test_3.csv")