...
processed_folder = "/Users/ansh/Music/AP/AP_Code/Major-Project/processed_data"
...
def get_user_input():
    try:
        ambient_temp = float(input("Enter Ambient Temp (C): "))
        solar_radiation = float(input("Enter Solar Radiation (W/m2): "))
        load_kw = float(input("Enter Load kW Sum (kW): "))
        inv_kw = float(input("Enter Inv kW Sum (kW): "))
        solar_kw = float(input("Enter Solar kW (kW): "))
        return pd.DataFrame([{
            'Ambient Temp (C)': ambient_temp,
            'Solar Radiation (W/m2)': solar_radiation,
            'Load kW Sum (kW)': load_kw,
            'Inv kW Sum (kW)': inv_kw,
            'Solar kW (kW)': solar_kw
        }])
    except ValueError:
